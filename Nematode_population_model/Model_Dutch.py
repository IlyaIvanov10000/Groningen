...
default_parameters = [12, 4000000, 0.5, 300, 0.00826, 0.00128, 0.001, 0.002, 0.0099, 0.00215, 0.0099, 0, 1, 0.5, 0.02]
...
def population_model(eggs_initial, dt, soil_type, K = default_parameters[1], b = default_parameters[2], eggs_per_cyst = default_parameters[3], Rae = default_parameters[4], Res = default_parameters[5], Rsc = default_parameters[6], Rcp = default_parameters[7], Ds = default_parameters[8], Rpa = default_parameters[9], Da = default_parameters[10], N = default_parameters[11], Rcr = default_parameters[12], Rrs = default_parameters[13], De = default_parameters[14], Dummy = default_parameters[0]):
    ...
    for i in range(total_time):
        ...
        T = Dummy #4.75 + 14.09 * np.sin(0.01057 * t + 0.3018)
        ...
        dX = 0.25 * Ws * (A + b * P)/K
        dE = eggs_per_cyst * Rae * T * A - Res * T * E * (1 - X) - De * E
...
def test_parameter_influence(start_range, end_range, steps, soil_type):
    data_points = [[],[]]
    for i in np.arange(start_range, end_range, steps):
        new_data = [i, population_model(1, 0.1, soil_type, Dummy = i)[1][1500]]
        data_points[0].append(new_data[0])
        data_points[1].append(new_data[1])
        
    return data_points
plot_dataset(test_parameter_influence(4, 24, 0.1, 'Loam')[0], test_parameter_influence(4, 24, 0.1, 'Loam')[1], 'Temperature (C)', 'Final egg density')
...
# plot_dataset(nlp_bifurcation(0, 1, 0.01, 0.0001, 10, 0.1, 'Loam')[0], nlp_bifurcation(0, 1, 0.01, 0.0001, 10, 0.1, 'Loam')[1], 'Recovery rate', 'Required RootPatch efficacy')
